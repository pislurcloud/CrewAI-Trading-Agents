...
def format_timegpt_forecast(forecast_df: pd.DataFrame, symbol: str, company_name: str) -> str:
    if forecast_df is None or forecast_df.empty:
        return "No TimeGPT forecast available."

    print(f"Debug - Available columns: {forecast_df.columns.tolist()}")
    print(f"Debug - DataFrame shape: {forecast_df.shape}")
    print(f"Debug - First few rows:\n{forecast_df.head()}")

    # Check what identifier column exists
    if 'unique_id' in forecast_df.columns:
        symbol_forecast = forecast_df[forecast_df['unique_id'] == symbol]
    elif 'symbol' in forecast_df.columns:
        symbol_forecast = forecast_df[forecast_df['symbol'] == symbol]
    elif 'ticker' in forecast_df.columns:
        symbol_forecast = forecast_df[forecast_df['ticker'] == symbol]
    else:
        # If no identifier column, assume the entire DataFrame is for our symbol
        print(f"Warning: No identifier column found. Using entire forecast DataFrame for {symbol}")
        symbol_forecast = forecast_df
    #symbol_forecast = forecast_df[forecast_df['unique_id'] == symbol]
